...
* **Unit Tests:** 50/50 passed (`Garden.UnitTests`); **Integration Tests:** 4/4 passed (`Garden.IntegrationTests`, ~7m26s) — *updated 2026-07-09, re-verified after `DEVELOPMENT_PLAN.md` Week 1 Day 1; was "17/17" at original completion*
* **Backend projects:** Garden.Core, Garden.Contracts, Garden.Shared, Garden.World, Garden.Engine, Garden.Infrastructure, Garden.Api — *updated 2026-07-09: `Garden.Simulation`, `Garden.Story`, `Garden.Tools` removed from the solution per `ADR-001` (they were orphaned placeholders, referenced by nothing); `Garden.Contracts` was omitted from this list at original completion despite already being part of the build graph*
...
* **Containers:** Dockerfile + docker-compose.yml (3 services: API, PostgreSQL, Observatory) — nginx COPY path bug fixed 2026-07-09, verified with `docker compose build`
* **API endpoints:** 40+ read-only endpoints across **17 controllers** — *updated 2026-07-09; was "11 controllers" at original completion, already stale by the time this doc was written*
...
*Updated 2026-07-09 — this section was stale (still showed the deleted `blueprint/` folder,
omitted `Garden.Contracts`, and the tree predates `ADR-001`'s removal of the three orphaned
placeholder projects). See `SPEC_INDEX.md` Change Log.*
...
├── docker-compose.yml
├── TheGarden.slnx
├── specification/       # Design documents (TG-000 through TG-DEV-009), SPEC_INDEX.md,
│                        # DEVELOPMENT_PLAN.md, ADR/, RFC/, Archive/
├── src/
│   ├── Garden.Api/          # ASP.NET Core Web API (controllers, hubs, Program.cs)
│   ├── Garden.Core/         # Domain foundation (events, identifiers, interfaces)
│   ├── Garden.Contracts/    # Reserved for future DTO/event-contract extraction (see ADR-001)
│   ├── Garden.Engine/       # Simulation engine (systems, services, generation)
│   ├── Garden.Infrastructure/ # Persistence, DI, backups, EF Core config
│   ├── Garden.Shared/       # Reserved for future math/random/collection utility extraction (see ADR-001)
│   ├── Garden.World/        # Domain entities, collections, world state
│   └── Garden.Observatory/  # React frontend (pages, components, hooks, API); owns its own nginx.conf
├── tests/
│   ├── Garden.UnitTests/        # 50 unit tests (deterministic generation, pathfinding, history archival, etc.)
│   └── Garden.IntegrationTests/ # 4 integration tests (full survival-simulation regression guards)
└── docs/
```
...
✓ **TG-003 Project Structure** — src/tests/specification structure, file-scoped namespaces, C# conventions — *updated 2026-07-09: `blueprint/` was deleted and replaced by `specification/`*
